...
class EmployeesController extends AppController
{
    ...
    public function employeeStock()
    {
        ...
        $employe = $this->request->query('employee_id');
        //$employe=$datas['employee_id'];
        ...
        if(!empty($to)){
            ...
            $row_material_list = $query
                ->contain(['Units','StockLedgers'=>function($query)use($employe){
                $totalInCase = $query->newExpr()
                    ->addCase(
                    $query->newExpr()->add(['status' => 'In','employee_id'=>$employe]),
                    $query->newExpr()->add(['quantity']),
                    'integer'
                    );
                    $totalOutCase = $query->newExpr()
                    ->addCase(
                        $query->newExpr()->add(['status' => 'Out','employee_id'=>$employe]),
                    $query->newExpr()->add(['quantity']),
                    'integer'
                    );
                    return $query->select([
    ...
    public function mainStock()
    {
        ...
        if(!empty($status)){ 
            $this->viewBuilder()->layout('excel_layout');	
                ...
                $dep=$this->request->query('department_id');
                $from=$this->request->query('from');;
                $to=$this->request->query('to');;
                $where =[];
                if(!empty($dep)){
                ...
                $query=$this->Employees->StockLedgers->RowMaterials->find();
                     ...
                     ->contain(['Units','StockLedgers'=>function($query)use($dep){
                      $totalInCase = $query->newExpr()
                         ->addCase(
                            $query->newExpr()->add(['status' => 'In','department_id'=>$dep]),
                            $query->newExpr()->add(['quantity']),
                            'integer'
                          );
                          $totalOutCase = $query->newExpr()
                          ->addCase(
                             $query->newExpr()->add(['status' => 'Out','department_id'=>$dep]),
                            $query->newExpr()->add(['quantity']),
                            'integer'
                          );
                           return $query->select([
