...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Issue Slip'), ['action' => 'edit', $issueSlip->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Issue Slip'), ['action' => 'delete', $issueSlip->id], ['confirm' => __('Are you sure you want to delete # {0}?', $issueSlip->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Issue Slips'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Issue Slip'), ['action' => 'add']) ?> </li>
...
<div class="issueSlips view large-9 medium-8 columns content">
    <h3><?= h($issueSlip->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Employee') ?></th>
            <td><?= $issueSlip->has('employee') ? $this->Html->link($issueSlip->employee->name, ['controller' => 'Employees', 'action' => 'view', $issueSlip->employee->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($issueSlip->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Voucher No') ?></th>
            <td><?= $this->Number->format($issueSlip->voucher_no) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created By') ?></th>
            <td><?= $this->Number->format($issueSlip->created_by) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Edited By') ?></th>
            <td><?= $this->Number->format($issueSlip->edited_by) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Is Deleted') ?></th>
            <td><?= $this->Number->format($issueSlip->is_deleted) ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= h($issueSlip->created_on) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Edited On') ?></th>
            <td><?= h($issueSlip->edited_on) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Issue Slip Rows') ?></h4>
        <?php if (!empty($issueSlip->issue_slip_rows)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($issueSlip->issue_slip_rows as $issueSlipRows): ?>
            <tr>
                ...
                <td><?= h($issueSlipRows->is_deleted) ?></td>
                <td class="actions">
    ...
    <div class="related">
        ...
        <?php if (!empty($issueSlip->stock_ledgers)): ?>
        <table cellpadding="0" cellspacing="0">
            <tr>
                <th scope="col"><?= __('Id') ?></th>
                <th scope="col"><?= __('Material Id') ?></th>
            ...
            <?php foreach ($issueSlip->stock_ledgers as $stockLedgers): ?>
            <tr>
